...
// ─── 狀態 ────────────────────────────────────
let isAdmin = false;
const ADMIN_PASSWORD = "kanri2026"; // ← 請更改此密碼
...
function productCard(p) {
  ...
  const priceLabel = p.price > 0 ? `TWD ${p.price.toLocaleString()}` : "洽談";
...
function saveProduct() {
  ...
  if (editingProductId) {
    ...
    if (idx !== -1) products[idx] = { ...products[idx], name, tags, desc, img, price, status };
  } else {
    products.push({ id: "p" + Date.now(), name, tags, desc, img, price, status });
  }
  ...
  container.innerHTML = cart.map(c => {
    ...
    return `
      <div class="cart-row">
        ...
        <span class="cart-row-price">${p.price > 0 ? "TWD " + subtotal.toLocaleString() : "洽談"}</span>
      </div>`;
  }).join("");

  const total = cart.reduce((s, c) => {
    const p = products.find(x => x.id === c.id);
    return s + (p && p.price > 0 ? p.price * c.qty : 0);
  }, 0);
  document.getElementById("cartTotal").textContent = total > 0 ? `合計：TWD ${total.toLocaleString()}` : "";
}
...
function checkout() {
  ...
  const hasNegotiable = cart.some(c => {
    const p = products.find(x => x.id === c.id);
    return p && p.price === 0;
  });
  if (hasNegotiable) {
  ...
  // 注意：AES + SHA256 加密需在後端完成以保護 HashKey/HashIV
  // 此處為前端示意，正式上線請務必移至後端 API
