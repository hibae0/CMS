...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    // 相容舊欄位名稱（notices → noticesHome, products → commissions）
    if (data.noticesHome)  noticesHome  = data.noticesHome;
    else if (data.notices) noticesHome  = data.notices;
    ...
    if (data.commissions)  commissions  = data.commissions;
      ...
      // 舊版 products 轉換成 commissions 格式
...
function renderCartModal() {
  ...
  container.innerHTML = cart.map(ci => {
    ...
    return `<div class="cart-row">
      ...
      <span class="cart-row-price" id="price_${ci.id}">${disp}</span>
    </div>`;
  }).join("");
...
function updateCartTotal() {
  ...
  cart.forEach(ci => {
    const p = commissions.find(x=>x.id===ci.id); if (!p) return;
    const pt = p.priceType||"fixed";
    if (pt==="fixed" && p.price > 0) {
      total += p.price * ci.qty;
    } else if (pt==="custom") {
      if (ci.customPrice > 0) {
        total += ci.customPrice * ci.qty;
        // 更新那筆的右側顯示
        const el = document.getElementById("price_"+ci.id);
